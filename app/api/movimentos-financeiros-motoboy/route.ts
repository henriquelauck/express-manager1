...
function normalizarTipo(valor: unknown): TipoMovimento | null {
  ...
  if (tipo === "CLIENTE" || tipo === "ESCRITORIO" || tipo === "AJUSTE") {
    return tipo;
  }
...
export async function GET() {
  try {
    ...
    return respostaErro("Não foi possível carregar os movimentos financeiros.", 500);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!motoboyId || !tipo) {
      return respostaErro("Motoboy e tipo são obrigatórios.", 400);
    }
    ...
    if (valor === 0) {
      return respostaErro("Informe um valor diferente de zero.", 400);
    }

    if (tipo === "ESCRITORIO" && valor < 0) {
      return respostaErro("Pagamentos do escritório não podem ter valor negativo.", 400);
    }
    ...
    const movimento = await prisma.movimentoFinanceiroMotoboy.create({
      data: {
        ...
        valor,
        descricao:
          descricao || (tipo === "ESCRITORIO" ? "Pagamento do escritório" : "Ajuste financeiro"),
        teleId: null,
        fechamentoId: null,
    ...
    return respostaErro("Não foi possível registrar o movimento financeiro.", 500);
  }
}
